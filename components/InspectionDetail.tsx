...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-industrial-900 border border-gray-700 p-3 rounded shadow-xl text-sm z-50">
...
export const InspectionDetail: React.FC<InspectionDetailProps> = ({ station }) => {
  ...
  if (!station) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-industrial-900 text-gray-600 px-8 text-center border-l border-gray-800">
         <div className="w-24 h-24 rounded-full bg-gray-800/50 flex items-center justify-center mb-6 animate-pulse">
            <Activity size={48} className="opacity-40" />
         </div>
         <h3 className="text-2xl font-bold text-gray-400 mb-2 tracking-wide">工位数据看板</h3>
         <p className="text-sm text-gray-500 max-w-[240px]">请点击左侧工位图，查看实时合格率趋势与单车检测明细。</p>
      </div>
    );
  }
  ...
  if (station.status === NodeStatus.INACTIVE || !station.children || station.children.length === 0) {
     return (
        <div className="h-full flex flex-col bg-industrial-900 border-l border-gray-800 shadow-[-20px_0_40px_rgba(0,0,0,0.6)] relative z-30">
            {/* Header Area (Minimal) */}
            <div className="px-6 py-6 border-b border-gray-800 bg-industrial-800/80 backdrop-blur-md flex-shrink-0 opacity-50">
                <div className="flex items-center justify-between mb-3">
                    <div className="text-xs text-gray-500 font-mono border border-gray-700 px-2 py-0.5 rounded bg-gray-800 tracking-wider">
                         {station.id.replace('asm-', '').toUpperCase()}
                    </div>
                    <StatusBadge status={station.status} size="sm" />
                </div>
                <h2 className="text-xl font-bold text-gray-400 tracking-tight leading-tight flex items-center gap-2">
                    {station.label}
                </h2>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-gray-600 px-8 text-center">
                ...
                <h3 className="text-xl font-bold text-gray-500 mb-1">暂无检测数据</h3>
                <p className="text-sm text-gray-600">该工位当前处于未激活状态或尚未配置检测项。</p>
            </div>
        </div>
     );
  }
  ...
  return (
    <div className="h-full flex flex-col bg-industrial-900 border-l border-gray-800 shadow-[-20px_0_40px_rgba(0,0,0,0.6)] relative z-30">
      ...
      {/* 1. Header Area */}
      <div className="px-6 py-6 border-b border-gray-800 bg-industrial-800/80 backdrop-blur-md flex-shrink-0">
          <div className="flex items-center justify-between mb-3">
             <div className="text-xs text-neon-blue font-mono border border-neon-blue/30 px-2 py-1 rounded bg-neon-blue/5 tracking-wider">
                {station.id.replace('asm-', '').toUpperCase()}
             </div>
          ...
          <div className="text-xs text-gray-500 mt-2 font-mono flex gap-4">
      ...
      <div className="h-[35%] min-h-[240px] p-6 border-b border-gray-800 bg-industrial-900/50 flex flex-col">
          <div className="flex items-center justify-between mb-6 px-2">
              <h3 className="text-sm font-bold text-gray-300 flex items-center gap-2">
                  <Activity size={16} className="text-neon-green" />
                  30天一次合格率趋势 (FPY)
              </h3>
              <span className="text-neon-green font-mono text-sm font-bold bg-neon-green/10 px-2 py-0.5 rounded">
                  Avg: 99.2%
              </span>
          </div>
          <div className="flex-1 w-full min-h-0">
            <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trendData}>
                    ...
                    <ReferenceLine y={98} stroke="#0aff00" strokeDasharray="3 3" strokeOpacity={0.4} label={{ position: 'right',  value: '目标值', fill: '#0aff00', fontSize: 10 }} />
                    <Area 
                        type="monotone" 
                        dataKey="value" 
                        stroke="#0aff00" 
          ...
          <div className="px-6 py-4 flex items-center gap-3 border-b border-gray-800">
             <div className="relative flex-1">
                 ...
                 <input 
                    ...
                    className="w-full bg-industrial-800 border border-gray-700 rounded-lg py-2 pl-10 pr-3 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-neon-blue/50 focus:ring-1 focus:ring-neon-blue/50 transition-all"
                 />
             </div>
             <button className="p-2 text-gray-400 hover:text-white bg-industrial-800 border border-gray-700 rounded-lg hover:bg-gray-700 transition-colors">
                <Filter size={16} />
             </button>
          </div>

          {/* Table Header */}
          <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-gray-800/50 text-xs font-bold text-gray-500 border-b border-gray-800">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-1">
             {filteredRecords.length > 0 ? (
                filteredRecords.map((row) => (
                    <div 
                        key={row.id} 
                        className="grid grid-cols-12 gap-4 items-center px-4 py-3 rounded border border-transparent hover:bg-white/5 hover:border-gray-700 transition-colors group"
                    >
                        {/* VIN */}
                        <div className="col-span-4 flex flex-col justify-center">
                            <span className="text-sm font-mono text-neon-blue group-hover:text-white transition-colors font-semibold">{row.vin}</span>
                            <span className="text-xs text-gray-600 mt-0.5">{row.time}</span>
                        </div>
                        
                        {/* Item Name */}
                        <div className="col-span-5 text-sm text-gray-300 leading-tight">
                            {row.itemName}
                        </div>
                        ...
                        <div className="col-span-3 flex justify-end">
                            {row.result === 'PASS' ? (
                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-neon-green/10 border border-neon-green/20 text-xs font-bold text-neon-green">
                                    <CheckCircle2 size={12} /> 合格
                                </span>
                            ) : (
                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-neon-red/10 border border-neon-red/20 text-xs font-bold text-neon-red animate-pulse">
                                    <XCircle size={12} /> 异常
                                </span>
                            )}
                        </div>
                ...
                <div className="flex flex-col items-center justify-center py-12 text-gray-600 gap-2">
                    ...
                    <span className="text-sm">未找到匹配的车辆记录</span>
                </div>
             )}
          </div>
      </div>
