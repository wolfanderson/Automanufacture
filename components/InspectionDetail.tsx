     ...
     return (
        <div className="h-full flex flex-col bg-industrial-800 border-l border-industrial-700 shadow-xl relative z-30">
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-gray-500 px-8 text-center bg-industrial-800">
                ...
                <p className="text-base text-gray-500">该工位当前处于未激活状态。</p>
            </div>
        </div>
     );
  }
    ...
    <div className="h-full flex flex-col bg-industrial-800 border-l border-industrial-700 shadow-2xl relative z-30">
      ...
      <div className="px-8 py-8 border-b border-industrial-700 bg-industrial-800 flex-shrink-0">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            <div className="bg-industrial-700/30 p-3 rounded-lg border border-industrial-600/60 hover:bg-industrial-700/50 transition-colors">
                <div className="flex items-center gap-2 mb-1.5">
                    <Box size={14} className="text-neon-blue" />
                    <span className="text-xs font-bold text-gray-400 font-mono tracking-wider">检测对象</span>
                </div>
                <div className="text-sm text-gray-200 font-medium leading-snug line-clamp-3">
                    {station.meta?.inspectionObject || '暂无详细描述'}
                </div>
            </div>
            <div className="bg-industrial-700/30 p-3 rounded-lg border border-industrial-600/60 hover:bg-industrial-700/50 transition-colors">
                <div className="flex items-center gap-2 mb-1.5">
                    <ScanLine size={14} className="text-neon-green" />
                    <span className="text-xs font-bold text-gray-400 font-mono tracking-wider">检测方案</span>
                </div>
                <div className="text-sm text-gray-200 font-medium leading-snug line-clamp-3">
          ...
          <div className="text-xs text-gray-500 mt-4 font-mono flex gap-4 pt-2 border-t border-industrial-700/30">
             <span>Items: <span className="text-gray-300 font-bold">{station.children?.length || 0}</span></span>
             <span className="w-px h-3 bg-industrial-600"></span>
             <span>Resp: <span className="text-gray-300 font-bold">张工 (8902)</span></span>
          </div>
      </div>

      {/* 2. Top Half: Pass Rate Trend Chart */}
      <div className="h-[30%] min-h-[200px] p-8 border-b border-industrial-700 bg-industrial-800 flex flex-col">
          ...
          <div className="flex-1 w-full min-h-0">
            <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trendData}>
                    ...
                    <ReferenceLine y={98} stroke="#0aff00" strokeDasharray="3 3" strokeOpacity={0.6} label={{ position: 'insideTopRight',  value: 'Target', fill: '#0aff00', fontSize: 10 }} />
                    <Area 
                        type="monotone" 
                        dataKey="value" 
                        stroke="#0aff00" 
          ...
          <div className="px-8 py-4 flex items-center gap-3 border-b border-industrial-700 bg-industrial-800">
             <div className="relative flex-1">
                 ...
                 <input 
                    ...
                    className="w-full bg-industrial-900 border border-industrial-600 rounded-lg py-2 pl-9 pr-4 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-neon-blue focus:ring-1 focus:ring-neon-blue transition-all"
                 />
             </div>
             <button className="p-2 text-gray-400 hover:text-white bg-industrial-900 border border-industrial-600 rounded-lg hover:bg-industrial-700 transition-colors">
                <Filter size={16} />
             </button>
          </div>

          {/* Table Header */}
          <div className="grid grid-cols-12 gap-2 px-8 py-2 bg-industrial-700/30 text-xs font-bold text-gray-400 border-b border-industrial-700 uppercase tracking-wider">
                ...
                filteredRecords.map((row) => (
                    ...
                    >
                        ...
                        <div className="col-span-5 flex flex-col justify-center">
                            <span className="text-xs font-mono text-neon-blue group-hover:text-white transition-colors font-bold tracking-wide">{row.vin}</span>
                        </div>
                        
                        {/* Time */}
                        <div className="col-span-4 flex items-center gap-1.5 text-xs text-gray-400 font-mono">
                        ...
                        <div className="col-span-3 flex justify-end">
                            {row.result === 'PASS' ? (
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-neon-green/5 border border-neon-green/10 text-[10px] font-bold text-neon-green">
                                    <CheckCircle2 size={10} /> PASS
                                </span>
                            ) : (
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-neon-red/5 border border-neon-red/10 text-[10px] font-bold text-neon-red animate-pulse">
